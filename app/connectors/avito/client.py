...
class AvitoClient:
    ...
    async def get_new_applications(self, account: Account, db: AsyncSession) -> List[Dict]:
        ...
        try:
            ...
            # ВАЖНО: Авито использует 'applies' в Job API v1
            app_list = resp_ids.get("applies", []) 
            if not app_list:
                return []
            ...
            # ИСПРАВЛЕНИЕ ТУТ: берем из 'applies', как на вашем скрине
            full_applications = details_resp.get("applies", []) 
